...
// ---------------------------------------------------------------------------
vec3 Calculate_DirectionalLight(LightParameters light, vec3 CameraPosition, Material material)
{

	// diffuse
	vec3  lightDirection = normalize(light.Direction);
	float diffuseCoeff = max(dot(ourNormal, -light.Direction), 0.0);
	vec3  resultDiffuse = diffuseCoeff * vec3(light.Diffuse) * vec3(material.diffuse);
	...
	vec3  specularPart = specularCoeff * vec3(light.Specular) * vec3(material.specular);
...
void main()
{
	...
	// Oswietlenie kierunkowe
	vec3  lightPart = Calculate_DirectionalLight(myLight, Camera_Position, material);

	// Cienie
	float shadowPart = Calculate_Shadow(ourPosLight, Light_Direction);

	// Kolor finalny
	vec3  finalColor = (myLight.Ambient + (1 - shadowPart) * lightPart) * texColor.rgb;
