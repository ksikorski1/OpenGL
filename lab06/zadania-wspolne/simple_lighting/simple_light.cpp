...
void DisplayScene()
{
	...
	// Nowe! obliczenie pozycji kamery
	// Funkcja ExtractCameraPos w pliku shader_stuff.hpp
	Camera_Position = ExtractCameraPos(Matrix_mv);
	...
	glUniform3fv( glGetUniformLocation( program[MODE], "Light_Specular" ), 1, &Light_Specular[0] );
	glUniform3fv( glGetUniformLocation( program[MODE], "Light_Position" ), 1, &Light_Position[0] );
	glUniform3fv( glGetUniformLocation( program[MODE], "Camera_Position" ), 1, &Camera_Position[0] );
...
void Initialize()
{
	...
	// 1. Programowanie potoku: tworzymy 4 programy
