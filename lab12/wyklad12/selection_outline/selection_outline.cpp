...
void DisplayScene()
{
    ...
    if (CollisionID >= 0)
    {
        ...
        glm::mat4 matModel = Table_of_Objects[CollisionID].matModel;
        Table_of_Objects[CollisionID].matModel = glm::scale(Table_of_Objects[CollisionID].matModel, glm::vec3(1.2, 1.2, 1.2));
        glUniform1i( glGetUniformLocation( program, "selected" ), 1);

            Table_of_Objects[CollisionID].Draw(program);

        glUniform1i( glGetUniformLocation( program, "selected" ), 0);
        Table_of_Objects[CollisionID].matModel = matModel;
